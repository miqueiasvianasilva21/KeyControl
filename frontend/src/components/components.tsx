...
export const Badge = ({ className = "", ...props }: any) => (
  <div className={`inline-flex items-center rounded-full border px-2.5 py-0.5 text-xs font-semibold transition-colors focus:outline-none focus:ring-2 focus:ring-slate-950 focus:ring-offset-2 ${className}`} {...props} />
);
...
// --- MODAIS (DIALOGS) ---
export const Dialog = ({ open, onOpenChange, children }: any) => open ? <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">{children}</div> : null;
export const DialogContent = ({ children }: any) => <div className="bg-white rounded-lg shadow-lg w-full max-w-md border border-slate-200 p-6 space-y-4">{children}</div>;
...
export const DialogFooter = ({ children }: any) => <div className="flex flex-col-reverse sm:flex-row sm:justify-end sm:space-x-2 mt-4">{children}</div>;
...
export const SelectItem = ({ value, children, disabled }: any) => <option value={value} disabled={disabled}>{children}</option>;
